---
title: "Arquitectura de Licencias de elichingon.com"
description: "Estructura de propiedad intelectual y licencias de contenido abierto de elichingon.com"
date: 2026-06-05
draft: false
showDate: false
showReadingTime: false
showWordCount: false
showAuthor: false
---

Este sitio es un ecosistema de contenido abierto, diseñado como un protocolo de información descentralizado sobre el I Ching. Para garantizar su máxima difusión y adopción, la propiedad intelectual del sitio está estructurada en capas independientes, cada una con su propio régimen legal.

## Capa 1 · Texto Original en Alemán (Dominio Público)

Los textos base del I Ching utilizados en este sitio corresponden a la traducción al alemán de Richard Wilhelm, publicada originalmente en 1924 bajo el título *I Ging, das Buch der Wandlungen*.

Richard Wilhelm falleció en 1930. Su obra original en alemán se encuentra en dominio público en la inmensa mayoría de las jurisdicciones del mundo (donde el copyright dura la vida del autor más 70 u 80 años). Por tanto, estos textos no están sujetos a derechos de autor y pertenecen al acervo común de la humanidad.

Se reconoce la autoría original a Richard Wilhelm en cada hexagrama y texto base utilizado.

## Capa 2 · Traducciones y Artículos Originales (CC BY 4.0)

Las traducciones al español, inglés y francés de los textos del I Ching (realizadas directamente a partir del original alemán de dominio público por Richard Wilhelm) y los artículos originales publicados en el sitio son obra de Dailingna Romero / elichingon.com.

Todo este contenido se distribuye bajo una licencia **Creative Commons Atribución 4.0 Internacional (CC BY 4.0)**.

Usted es libre de:
- **Compartir** - copiar y redistribuir el material en cualquier medio o formato, para cualquier propósito, incluso comercial.
- **Adaptar** - remezclar, transformar y construir a partir del material para cualquier propósito, incluso comercial.

Bajo la siguiente condición:
- **Atribución** - Debe reconocer la autoría de manera adecuada, proporcionar un enlace a la licencia, e indicar si se realizaron cambios. Puede hacerlo de cualquier manera razonable, pero no de una manera que sugiera que elichingon.com lo apoya a usted o al uso que le dé.

Esta licencia aplica específicamente a:
- Todos los artículos y ensayos originales publicados en el sitio.
- Las traducciones al español, inglés y francés de los textos del I Ching.
- El diseño editorial y la curaduría de contenidos.

## Capa 3 · Contenido de Dominio Público y Conocimiento Tradicional

Este sitio incluye contenido que no es objeto de derechos de autor por ser conocimiento tradicional o dominio público:

- Los 64 hexagramas del I Ching y sus textos base (conocidos desde la antigüedad).
- Los ordenamientos Cielo Anterior (Fu Xi) y Cielo Posterior (Rey Wen) de los trigramas.
- La Tabla Avanzada de Correspondencias: Las correspondencias entre trigramas, elementos, direcciones, estaciones, familia, animales, partes del cuerpo, etc., son conocimiento tradicional documentado en la literatura clásica del I Ching durante siglos. La presentación de estas correspondencias en formato de tabla en este sitio es una compilación de conocimiento de dominio público, no una obra original sujeta a derechos de autor.

Este contenido puede ser utilizado libremente por cualquier persona sin restricciones.

## Capa 4 · Plataforma y Tema (Software de Terceros)

Este sitio está construido con [Hugo](https://gohugo.io/), un generador de sitios estáticas de código abierto bajo licencia [Apache 2.0](https://www.apache.org/licenses/LICENSE-2.0).

El tema visual utilizado es [Blowfish](https://github.com/nunocorvalho/blowfish), desarrollado por [Nuno Corvalho](https://github.com/nunocorvalho), distribuido bajo licencia [MIT](https://opensource.org/licenses/MIT).

Agradecemos a los desarrolladores de estas herramientas de código abierto que hacen posible este proyecto.

## Capa 5 · Verificación Criptográfica de Autoría

Todo el contenido publicado bajo la Capa 2 (traducciones y artículos originales de Dailingna Romero) está acompañado de una **firma criptográfica** publicada en la red descentralizada [Nostr](https://nostr.com).

Esta firma cumple tres funciones:

1. **Prueba de autoría**: Demuestra matemáticamente que el contenido fue creado por la identidad digital `dailingna@elichingon.com`.
2. **Estampa temporal inmutable**: Registra la fecha y hora exacta de publicación original, estableciendo prioridad temporal en caso de plagio o disputa.
3. **Verificación independiente**: Cualquier persona puede verificar la autenticidad del contenido sin depender de plataformas centralizadas, abogados o tribunales.

### ¿Por qué es importante?

Dailingna Romero es un avatar digital, no una persona física ni jurídica. Esto significa que no puede ejercer acciones legales tradicionales por violación de derechos de autor. Sin embargo, la firma criptográfica proporciona una **prueba matemática e inmutable de autoría** que es verificable por cualquier persona con acceso a internet.

Si alguien copia, plagia o presenta como propio el contenido de este sitio sin atribución, existe evidencia criptográfica verificable de que Dailingna Romero lo publicó primero.

### ¿Cómo verificar?

Cada artículo firmado incluye un bloque de verificación al final del contenido con un enlace a la nota de Nostr correspondiente. Para más detalles sobre el proceso técnico de verificación, consulta la página de [verificación de autenticidad](https://elichingon.com/verificar/).

**Identidad verificada**: `dailingna@elichingon.com`  
**Protocolo**: [Nostr](https://nostr.com) / [NIP-05](https://github.com/nostr-protocol/nips/blob/master/05.md)

## Sobre los Derechos Morales

Independientemente de las licencias aplicadas, se respetan y exigen los derechos morales de autor reconocidos por la legislación internacional. El uso del contenido no debe realizarse de manera que perjudique el honor o la reputación del autor, ni presentarse como obra propia sin la atribución correspondiente.

## Formato de Atribución Recomendado

Si utiliza nuestros contenidos, le agradecemos usar el siguiente formato:

**Para artículos y traducciones:**
> "Texto original de Richard Wilhelm (dominio público). Traducción al español y artículos por Dailingna Romero / [elichingon.com](https://elichingon.com), utilizados bajo licencia [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)."

**Para citar un artículo específico:**
> "Romero, D. (2026). [Título del artículo]. elichingon.com. Licencia CC BY 4.0."

## Contacto para Dudas sobre Licenciamiento

Si desea utilizar contenido de este sitio de una forma no contemplada en estas licencias, o tiene dudas sobre cómo realizar la atribución correctamente, puede contactar a través de:

- **Email**: [contacto@elichingon.com](mailto:contacto@elichingon.com)
- **Nostr**: `dailingna@elichingon.com`

---

**Última actualización**: 7 de junio de 2026